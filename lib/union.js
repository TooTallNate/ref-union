  ...
  function UnionType (arg, data) {
    ...
    if (Buffer.isBuffer(arg)) {
      ...
      assert(arg.length >= UnionType.size, 'Buffer instance must be at least '
          + UnionType.size + ' bytes to back this struct type')
      store = arg
      arg = data
    } else {
...
function defineProperty (name, type) {
  ...
  function get () {
    ...
    return ref.get(this._pointer, 0, type)
  }

  function set (value) {
    debug('setting "%s" union field (length: %d)', name, type.size, value)
    return ref.set(this._pointer, 0, value, type)
  }
}
...
function recalc (union) {
  ...
  if (left > 0) {
    ...
    struct.size += union.alignment - left
  }
}
